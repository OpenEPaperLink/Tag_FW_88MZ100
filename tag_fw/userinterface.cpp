...
void showSplashScreen() {
    ...
    switch (tagProfile.xRes) {
        ...
        case 400: {
            ...
            fr.setFont((char *)"font/FreeSansBold18pt7b");
            if (tagProfile.bpp == 2) {
                fr.epdPrintf(15, 66, 1, rotation::ROTATE_0, "M2 4.2\" BWR");
            }
            fr.setFont((char *)"font/FreeSans9pt7b");
            fr.epdPrintf(10, 264, 0, rotation::ROTATE_0, "Tag MAC: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", mSelfMac[7], mSelfMac[6], mSelfMac[5], mSelfMac[4], mSelfMac[3], mSelfMac[2], mSelfMac[1], mSelfMac[0]);
            addQR(80, 120, 3, 3, "https://openepaperlink.eu/tag/0/%02X/%02X%02X%02X%02X%02X%02X%02X%02X/", tag.OEPLtype, mSelfMac[7], mSelfMac[6], mSelfMac[5], mSelfMac[4], mSelfMac[3], mSelfMac[2], mSelfMac[1], mSelfMac[0]);
            fr.epdPrintf(382, 180, 0, rotation::ROTATE_270, "v%04X%s", FW_VERSION, FW_VERSION_SUFFIX);
        } break;
    }
...
void showAPFound() {
    ...
    switch (tagProfile.xRes) {
        ...
        case 400: {
            ...
            fr.epdPrintf(5, 5, 0, rotation::ROTATE_0, (char *)"OpenEPaperLink");
            fr.setFont((char *)"font/FreeSans9pt7b");
            fr.epdPrintf(10, 64, 0, rotation::ROTATE_0, "AP Found at channel %d", currentChannel);
            fr.epdPrintf(10, 90, 0, rotation::ROTATE_0, "AP MAC: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", APmac[7], APmac[6], APmac[5], APmac[4], APmac[3], APmac[2], APmac[1], APmac[0]);
            fr.epdPrintf(10, 215, 0, rotation::ROTATE_0, "Battery: %d.%dV", batteryVoltage / 1000, batteryVoltage % 1000);
            fr.epdPrintf(10, 250, 0, rotation::ROTATE_0, "Tag MAC: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", mSelfMac[7], mSelfMac[6], mSelfMac[5], mSelfMac[4], mSelfMac[3], mSelfMac[2], mSelfMac[1], mSelfMac[0]);
            fr.epdPrintf(382, 180, 0, rotation::ROTATE_270, "v%04X%s", FW_VERSION, FW_VERSION_SUFFIX);
        }
    }
    addOverlay();
...
void showNoAP() {
    ...
    switch (tagProfile.xRes) {
        ...
        case 400: {
            ...
            fr.epdPrintf(5, 5, 0, rotation::ROTATE_0, (char *)"No AP Found!");
            fr.setFont((char *)"font/FreeSans9pt7b");
            fr.epdPrintf(20, 64, 0, rotation::ROTATE_0, "We'll try again later");
            fr.epdPrintf(10, 276, 0, rotation::ROTATE_0, "Tag MAC: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", mSelfMac[7], mSelfMac[6], mSelfMac[5], mSelfMac[4], mSelfMac[3], mSelfMac[2], mSelfMac[1], mSelfMac[0]);
